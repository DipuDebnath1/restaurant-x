...
const Payment = () => {
    ...
    const getQuantity = (id) =>{
        const data = localdataInfo.find(item=>item.id==id)
        return data.quantity
    }
    const geProductPrice = (id,price) =>{
        const data = localdataInfo.find(item=>item.id==id)
        const productPrice = (data.quantity*price)
        return productPrice
    }
                    ...
                    <div className="w-full md:w-2/3">
                        <div className="bg-white p-4 rounded shadow_body_order">
                            ...
                            <ul className="list-disc space-y-2">
                                ...
                                {
                                    addTocart.map(item => <li key={item.id} className="flex justify-between items-center p-2">{item.title}
                                        ...
                                        <span>
                                            {
                                            geProductPrice(item.id,item.price)
                            ...
                            <div className="flex justify-end">
                                <span className="text-gray-700 font-bold">Total: 00</span>
                                {/* <span className="text-gray-700 ml-2">${setAllproductPrice}</span> */}
                            </div>
                        </div>
                    </div>
